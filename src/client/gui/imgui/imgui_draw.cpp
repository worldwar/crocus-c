...
#elif (defined(__FreeBSD__) || defined(FreeBSD_kernel) ||                      \
...
#endif
#include "client/gui/imgui/stb_rect_pack.h"
...
#endif
#include "client/gui/imgui/stb_truetype.h"
        ...
        if (i == 0)
            memset(&_Channels[0], 0,
                   sizeof(_Channels[0])); // channel 0 is a copy of
                                          // CmdBuffer/IdxBuffer, don't destruct
                                          // again
...
void ImDrawList::PushClipRectFullScreen() {
    ...
    // PushClipRect(GetVisibleRect());   // FIXME-OPT: This would be more
    // correct but we're not supposed to access ImGuiContext from here?
}
...
void ImDrawList::PathArcTo(const ImVec2 &centre, float radius, float amin,
                           float amax, int num_segments) {
    if (radius == 0.0f)
        _Path.push_back(centre);
...
void ImDrawList::PathRect(const ImVec2 &a, const ImVec2 &b, float rounding,
    ...
    float r = rounding;
    r = ImMin(r, fabsf(b.x - a.x) *
                         (((rounding_corners & corners_top) == corners_top) ||
                                  ((rounding_corners & corners_bottom) ==
                                   corners_bottom)
                              ? 0.5f
                              : 1.0f) -
                     1.0f);
    r = ImMin(r, fabsf(b.y - a.y) *
                         (((rounding_corners & corners_left) == corners_left) ||
                                  ((rounding_corners & corners_right) ==
                                   corners_right)
                              ? 0.5f
                              : 1.0f) -
                     1.0f);
    ...
    if (r <= 0.0f || rounding_corners == 0) {
        ...
        const float r2 =
            (rounding_corners & ImGuiCorner_BottomRight) ? r : 0.0f;
...
void ImDrawList::AddText(const ImFont *font, float font_size, const ImVec2 &pos,
    ...
    // IMPORTANT: This is one of the few instance of breaking the encapsulation
    // of ImDrawList, as we pull this from ImGui state, but it is just SO
    // useful. Might just move Font/FontSize to ImDrawList?
    if (font == NULL)
        font = GImGui->Font;
    if (font_size == 0.0f)
...
void ImFontAtlas::CustomRectCalcUV(const CustomRect *rect, ImVec2 *out_uv_min,
                                   ImVec2 *out_uv_max) {
...
bool ImFontAtlasBuildWithStbTruetype(ImFontAtlas *atlas) {
    ...
    for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++) {
        ImFontConfig &cfg = atlas->ConfigData[input_i];
        ImFontTempBuildData &tmp = tmp_array[input_i];
        ImFont *dst_font = cfg.DstFont; // We can have multiple input fonts
                                        // writing into a same destination font
                                        // (when using MergeMode=true)
        ...
        dst_font->FallbackGlyph = NULL; // Always clear fallback so FindGlyph
                                        // can return NULL. It will be set again
                                        // in BuildLookupTable()
        for (int i = 0; i < tmp.RangesCount; i++) {
            ...
            for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1) {
                ...
                glyph.XAdvance =
                    (pc.xadvance +
                     cfg.GlyphExtraSpacing.x); // Bake spacing into XAdvance

                if (cfg.PixelSnapH)
                    glyph.XAdvance = (float)(int)(glyph.XAdvance + 0.5f);
                dst_font->MetricsTotalSurface +=
                    (int)((glyph.U1 - glyph.U0) * atlas->TexWidth + 1.99f) *
                    (int)((glyph.V1 - glyph.V0) * atlas->TexHeight +
                          1.99f); // +1 to account for average padding, +0.99 to
                                  // round
            }
        }
...
void ImFontAtlasBuildRegisterDefaultCustomRects(ImFontAtlas *atlas) {
    // FIXME-WIP: We should register in the constructor (but cannot because our
    // static instances may not have allocator ready by the time they
    // initialize). This needs to be fixed because we can expose CustomRects.
    if (atlas->CustomRects.empty())
        atlas->CustomRectRegister(FONT_ATLAS_DEFAULT_TEX_DATA_ID,
...
const ImWchar *ImFontAtlas::GetGlyphRangesJapanese() {
    ...
    // encoding helps us reduce the source code size.
...
const char *ImFont::CalcWordWrapPositionA(float scale, const char *text,
    ...
    while (s < text_end) {
        ...
        const float char_width =
            ((int)c < IndexXAdvance.Size ? IndexXAdvance[(int)c]
                                         : FallbackXAdvance);
        if (ImCharIsSpace(c)) {
            if (inside_word) {
    ...
    while (s < text_end) {
        if (word_wrap_enabled) {
            ...
            if (!word_wrap_eol) {
                ...
                if (word_wrap_eol == s) // Wrap_width is too small to fit
