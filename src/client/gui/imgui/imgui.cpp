...
// your codebase. Get latest version at https://github.com/ocornut/imgui
// Releases change-log at https://github.com/ocornut/imgui/releases
// Gallery (please post your screenshots/video there!):
// https://github.com/ocornut/imgui/issues/772 Developed by Omar Cornut and
...
/*
 ...
 without a label? (Yes). A primer on labels/IDs.
   - How can I tell when ImGui wants my mouse/keyboard inputs VS when I can pass
 ...
 Korean, Cyrillic?
   - How can I preserve my ImGui context across reloading a DLL? (loss of the
 ...
 - Add the ImGui source files to your projects, using your preferred build
 ...
 examples/.
 - When using ImGui, your programming IDE if your friend: follow the declaration
 of variables, functions and types to find comments about them.
 ...
 is called.
 - ImGui never touches or knows about your GPU state. the only function that
 ...
 around until it hits your render function. ImGui knows nothing about what those
 bits represent, it just passes them around. It is up to you to decide what you
 want the void* to carry! It could be an identifier to your OpenGL texture (cast
 GLuint to void*), a pointer to your custom engine material (cast MyMaterial* to
 void*), etc. At the end of the chain, your renderer takes this void* to cast it
 back into whatever it needs to select a current texture to render. Refer to
 examples applications, where each renderer (in a imgui_impl_xxxx.cpp file) is
 treating ImTextureID as a different thing. (c++ tip: OpenGL uses integers to
 identify textures. You can safely store an integer into a void*, just cast it
 to void*, don't take it's address!) To display a custom image/texture within an
 ImGui window, you may use ImGui::Image(), ImGui::ImageButton(),
 ImDrawList::AddImage() functions. ImGui will generate the geometry and draw
 calls using the ImTextureID that you passed and which your renderer can use. It
 is your responsibility to get textures uploaded to your GPU.
 ...
 label? A: Yes. A primer on the use of labels/IDs in ImGui..

   - Elements that are not clickable, such as Text() items don't need an ID.

   - Interactive widgets require state to be carried over multiple frames (most
 typically ImGui often needs to remember what is the "active" widget). to do so
 they need a unique ID. unique ID are typically derived from a string label, an
 integer index or a pointer.
 ...
 Q: How can I tell when ImGui wants my mouse/keyboard inputs VS when I can pass
 them to my application? A: You can read the
 ...
 expect to interact with either ImGui or your application during the same frame
 when that transition occurs. ImGui is tracking dragging and widget activity
 that may occur outside the boundary of a window, so 'io.WantCaptureMouse' is
 more accurate and correct than checking if a window is hovered. (Advanced note:
 text input releases focus on Return 'KeyDown', so the following Return 'KeyUp'
 event that your application receive will typically have
 'io.WantCaptureKeyboard=false'. Depending on your application logic it may or
 not be inconvenient. You might want to track which key-downs were for ImGui
 (e.g. with an array of bool) and filter out the corresponding key-ups.)
 ...
 Q: How can I preserve my ImGui context across reloading a DLL? (loss of the
 global/static variables) A: Create your own context 'ctx = CreateContext()' +
...
#include "client/gui/imgui/imgui_internal.h"
...
ImGuiStyle::ImGuiStyle() {
    ...
    Colors[ImGuiCol_FrameBg] =
        ImVec4(0.80f, 0.80f, 0.80f, 0.30f); // Background of checkbox, radio
                                            // button, plot, slider, text input
...
bool ImGui::ItemAdd(const ImRect &bb, const ImGuiID *id) {
    ...
    window->DC.LastItemHoveredAndUsable = window->DC.LastItemHoveredRect =
        false;
    ...
    if (IsMouseHoveringRect(bb.Min, bb.Max)) {
        ...
        // ActiveId==window->MoveID (we clicked on the window background) So
        // that clicking on items with no active id such as Text() still returns
...
bool ImGui::FocusableItemRegister(ImGuiWindow *window, bool is_active,
    ...
    if (tab_stop && window->FocusIdxAllRequestNext == INT_MAX &&
        ...
        // Modulo on index will be applied at the end of frame once we've got
        // the total counter of items.
...
void ImGui::NewFrame() {
    ...
    if ((g.IO.MousePos.x < 0 && g.IO.MousePos.y < 0) ||
        (g.IO.MousePosPrev.x < 0 &&
         g.IO.MousePosPrev.y < 0)) // if mouse just appeared or disappeared
                                   // (negative coordinate) we cancel out
    ...
    // Handle user moving window (at the beginning of the frame to avoid input
    // lag or sheering). Only valid for root windows.
    ...
    // Find the window we are hovering. Child windows can extend beyond the
    // limit of their parent so we need to derive HoveredRootWindow from
    // HoveredWindow
    g.HoveredWindow =
    ...
    // Are we using inputs? Tell user so they can capture/discard the inputs
    // away from the rest of their application. When clicking outside of a
    // window we assume the click is owned by the application and won't request
    // capture. We need to track click ownership.
    ...
    if (g.HoveredWindow && g.IO.MouseWheel != 0.0f &&
        ...
        if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
            ...
            SetWindowScrollY(window,
                             window->Scroll.y - g.IO.MouseWheel *
                                                    window->CalcFontSize() *
                                                    scroll_lines);
        }
    }
...
static void SaveIniSettingsToDisk(const char *ini_filename) {
    ...
    for (int i = 0; i != g.Windows.Size; i++) {
        ...
        if (!settings) // This will only return NULL in the rare instance where
                       // the window was first created with
...
static void AddDrawListToRenderList(ImVector<ImDrawList *> &out_render_list,
    ...
    // per window) If this assert triggers because you are drawing lots of stuff
    // manually, you can: A) Add '#define ImDrawIdx unsigned int' in imconfig.h
    // to set the index size to 4 bytes. You'll need to handle the 4-bytes
    // indices to your renderer.
...
void ImGui::EndFrame() {
    ...
    IM_ASSERT(g.FrameCountEnded != g.FrameCount); // ImGui::EndFrame() called
                                                  // multiple times, or forgot
                                                  // to call ImGui::NewFrame()
                                                  // again
    ...
    if (g.Style.Alpha > 0.0f) {
        ...
        for (int i = 0; i != g.Windows.Size; i++) {
            ...
            if (window->Active && window->HiddenFrames <= 0 &&
                ...
                // FIXME: Generalize this with a proper layering system so e.g.
                // user can draw in specific layers, below text, ..
...
void ImGui::RenderCheckMark(ImVec2 pos, ImU32 col) {
    ...
    window->DrawList->PathLineTo(
        ImVec2(bx + rem_third * 2, by - rem_third * 2));
...
ImVec2 ImGui::CalcTextSize(const char *text, const char *text_end,
    ...
    // baked into glyph->XAdvance field)
...
static ImGuiWindow *FindHoveredWindow(ImVec2 pos, bool excluding_childs) {
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--) {
        ...
        // Using the clipped AABB so a child window will typically be clipped by
        // its parent.
        ...
        if (ImGuiWindow *window = ImGui::FindWindowByName(window_name))
            if (window->Active) {
                ...
                ImFormatString(window_name, IM_ARRAYSIZE(window_name),
                               "##Tooltip%02d", ++g.TooltipOverrideCount);
            }
...
static bool BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags) {
    ...
    if (flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(
            name, IM_ARRAYSIZE(name), "##menu_%d",
...
void ImGui::EndChild() {
    ...
    if ((window->Flags & ImGuiWindowFlags_ComboBox) || window->BeginCount > 1) {
        ...
        if (window->AutoFitChildAxises & 0x01) // Arbitrary minimum zero-ish
...
static ImVec2 FindBestPopupWindowPos(const ImVec2 &base_pos, const ImVec2 &size,
    ...
    // Clamp into visible area while not overlapping the cursor. Safety padding
    // is optional if our popup size won't fit without it.
    ...
    ImVec2 base_pos_clamped =
        ImClamp(base_pos, r_outer.Min, r_outer.Max - size);
...
bool ImGui::Begin(const char *name, bool *p_open,
    ...
    const bool window_appearing_after_being_hidden =
        (window->HiddenFrames == 1);
    ...
    if (g.SetNextWindowPosCond) {
        ...
        if (window_pos_set_by_api &&
            ...
            window->SetWindowPosCenterWanted =
                true; // May be processed on the next frame if this is our first
                      // frame and we are measuring size
    ...
    for (root_idx = g.CurrentWindowStack.Size - 1; root_idx > 0; root_idx--)
        if (!(g.CurrentWindowStack[root_idx]->Flags &
    ...
    window->RootNonPopupWindow =
        g.CurrentWindowStack[root_non_popup_idx]; // This is merely for
                                                  // displaying the
                                                  // TitleBgActive color.
    ...
    if (first_begin_of_the_frame) {
        ...
        if (!window_was_active) {
            // Popup first latch mouse position, will position itself when it
            // appears next frame
        ...
        // Save contents size from last frame for auto-fitting (unless
        ...
        // Lock window padding so that altering the ShowBorders flag for
        // children doesn't have side-effects.
        ...
        if ((flags & ImGuiWindowFlags_Tooltip) != 0) {
            // Tooltip always resize. We keep the spacing symmetric on both
            // axises for aesthetic purpose.
            ...
            // Handling case of auto fit window not fitting in screen on one
            // axis, we are growing auto fit size on the other axis to
            // compensate for expected scrollbar. FIXME: Might turn bigger than
            ...
            size_auto_fit.y =
                ImMax(size_auto_fit.y - style.ItemSpacing.y, 0.0f);
        }

        // Handle automatic resize
        if (window->Collapsed) {
            // We still process initial auto-fit on collapsed windows to get a
            // window width, But otherwise we don't honor
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) &&
            ...
            window->Size = window->SizeFull =
                size_on_first_use; // NB: argument name 'size_on_first_use'
                                   // misleading here, it's really just 'size'
                                   // as provided by user passed via
                                   // BeginChild()->Begin().
        }
        ...
        if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api) {
            ImRect rect_to_avoid(
                ...
                g.IO.MousePos.y + 24); // FIXME: Completely hard-coded. Perhaps
                                       // center on cursor hit-point instead?
            window->PosFloat = FindBestPopupWindowPos(
                g.IO.MousePos, window->Size, &window->AutoPosLastDirection,
                rect_to_avoid);
            if (window->AutoPosLastDirection == -1)
                window->PosFloat =
                    g.IO.MousePos + ImVec2(2, 2); // If there's not enough room,
                                                  // for tooltip we prefer
                                                  // avoiding the cursor at all
                                                  // cost even if it means that
                                                  // part of the tooltip won't
                                                  // be visible.
        }
        ...
        if (window->Collapsed) {
            ...
            ImU32 resize_col = 0;
            const float resize_corner_size = ImMax(
                g.FontSize * 1.35f, window_rounding + 1.0f + g.FontSize * 0.2f);
            if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) &&
                window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 &&
                !(flags & ImGuiWindowFlags_NoResize)) {
                // Manual resize
                const ImVec2 br = window->Rect().GetBR();
                const ImRect resize_rect(br -
                                             ImVec2(resize_corner_size * 0.75f,
                                                    resize_corner_size * 0.75f),
                                         br);
                ...
                if (g.HoveredWindow == window && held &&
                    g.IO.MouseDoubleClicked[0]) {
                    ...
                    // We don't use an incremental MouseDelta but rather compute
                    // an absolute target size based on mouse position
                    ApplySizeFullWithConstraint(
                        window, (g.IO.MousePos - g.ActiveIdClickOffset +
                                 resize_rect.GetSize()) -
                                    window->Pos);
                    MarkIniSettingsDirty(window);
                }
            ...
            window->ScrollbarY =
                ...
                ((window->SizeContents.y >
                  window->Size.y + style.ItemSpacing.y) &&
                 !(flags & ImGuiWindowFlags_NoScrollbar));
            window->ScrollbarX =
            ...
            window->BorderSize =
                (flags & ImGuiWindowFlags_ShowBorders) ? 1.0f : 0.0f;
            ...
            ImVec4 bg_color =
                style.Colors[bg_color_idx]; // We don't use GetColorU32()
                                            // because bg_alpha is assigned (not
                                            // multiplied) below
            if (bg_alpha >= 0.0f)
                bg_color.w = bg_alpha;
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(
                    ...
                    GetColorU32(
                        (g.NavWindow && window->RootNonPopupWindow ==
            ...
            if (flags & ImGuiWindowFlags_ShowBorders) {
                ...
                if (!(flags & ImGuiWindowFlags_NoTitleBar))
                    window->DrawList->AddLine(
                        title_bar_rect.GetBL() + ImVec2(1, 0),
    ...
    // rectangle doesn't lag by a frame Note that if our window is collapsed we
    // will end up with a null clipping rectangle which is the correct behavior.
    ...
    clip_rect.Max.x = ImFloor(
        0.5f + window->Pos.x + window->Size.x - window->ScrollbarSizes.x -
        ImMax(border_size, ImFloor(window->WindowPadding.x * 0.5f)));
...
static void Scrollbar(ImGuiWindow *window, bool horizontal) {
    ...
    const ImU32 grab_col =
        ImGui::GetColorU32(held ? ImGuiCol_ScrollbarGrabActive
...
static const ImGuiStyleVarInfo GStyleVarInfo[ImGuiStyleVar_Count_] = {
    ...
    {ImGuiDataType_Float,
     (ImU32)IM_OFFSETOF(
         ImGuiStyle, ChildWindowRounding)}, // ImGuiStyleVar_ChildWindowRounding
    {ImGuiDataType_Float2,
     (ImU32)IM_OFFSETOF(ImGuiStyle,
...
void ImGui::SetNextWindowContentWidth(float width) {
    ...
    g.SetNextWindowContentSizeVal = ImVec2(
        width, g.SetNextWindowContentSizeCond ? g.SetNextWindowContentSizeVal.y
                                              : 0.0f);
    g.SetNextWindowContentSizeCond = ImGuiCond_Always;
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
    ...
    if (display_frame) {
        ...
        RenderCollapseTriangle(
            bb.Min + padding + ImVec2(0.0f, text_base_offset_y), is_open, 1.0f);
        if (g.LogEnabled) {
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect &aabb,
    ...
    // to allow for a TAB focus to happen)
    ...
    if (g.ActiveId == id) {
        if (g.IO.MouseDown[0]) {
            ...
            if (fabsf(mouse_drag_delta.x - g.DragLastMouseDelta.x) > 0.0f) {
                ...
                float adjust_delta =
                    (mouse_drag_delta.x - g.DragLastMouseDelta.x) * speed;
...
namespace ImGuiStb {
    ...
    static int STB_TEXTEDIT_MOVEWORDLEFT_IMPL(STB_TEXTEDIT_STRING *obj,
                                              int idx) {
        idx--;
...
#include "client/gui/imgui/stb_textedit.h"
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size,
    ...
    if (g.ActiveId == id) {
        ...
        const bool osx_double_click_selects_words =
            io.OSXBehaviors; // OS X style: Double click selects by word instead
                             // of selecting whole text
        ...
        if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
            ...
            if (!edit_state.HasSelection()) {
                if (is_wordmove_key_down)
                    edit_state.OnKeyPressed(STB_TEXTEDIT_K_WORDLEFT |
                                            STB_TEXTEDIT_K_SHIFT);
                else if (io.OSXBehaviors && io.KeySuper && !io.KeyAlt &&
                         !io.KeyCtrl)
        ...
        if (cancel_edit) {
            ...
            if (is_editable &&
                strcmp(edit_state.TempTextBuffer.Data, buf) != 0) {
...
bool ImGui::Combo(const char *label, int *current_item,
    ...
    if (IsPopupOpen(id)) {
        ...
        if ((popup_y2 - popup_y1) <
            ImMin(popup_height,
                  frame_bb.Min.y - style.DisplaySafeAreaPadding.y)) {
            // Position our combo ABOVE because there's more space to fit!
            // (FIXME: Handle in Begin() or use a shared helper. We have similar
            // code in Begin() for popup placement)
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        RenderCollapseTriangle(pos + ImVec2(window->MenuColumns.Pos[2] +
                                                extra_w + g.FontSize * 0.20f,
                                            0.0f),
                               false);
        if (!enabled)
            PopStyleColor();
    ...
    } else if (menu_is_open && pressed && menuset_is_open) // Menu bar: click an
                                                           // open menu again to
                                                           // close it
    {
        want_close = true;
        want_open = menu_is_open = false;
                           ...
                           !menu_is_open)) // menu-bar: first click to open,
                                           // then hover to open others
        want_open = true;
...
void ImGui::ColorTooltip(const char *text, const float col[4],
    ...
    if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF) {
        ...
        for (float y = p_min.y + grid_off.y; y < p_max.y;
            ...
            for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step;
                ...
                if (y1 <= p_min.y) {
                    if (x1 <= p_min.x)
                        rounding_corners_flags_cell |= ImGuiCorner_TopLeft;
                    if (x2 >= p_max.x)
                        rounding_corners_flags_cell |= ImGuiCorner_TopRight;
                }
                if (y2 >= p_max.y) {
                    if (x1 <= p_min.x)
                        rounding_corners_flags_cell |= ImGuiCorner_BottomLeft;
                    if (x2 >= p_max.x)
                        rounding_corners_flags_cell |= ImGuiCorner_BottomRight;
                }
...
bool ImGui::ColorButton(const char *desc_id, const ImVec4 &col,
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
        ...
        RenderColorRectWithAlphaCheckerboard(
            ...
            ImGuiCorner_TopRight | ImGuiCorner_BottomRight);
        window->DrawList->AddRectFilled(
            bb.Min, ImVec2(mid_x, bb.Max.y), GetColorU32(col_without_alpha),
            rounding, ImGuiCorner_TopLeft | ImGuiCorner_BottomLeft);
    } else {
        RenderColorRectWithAlphaCheckerboard(
            bb.Min, bb.Max,
...
static void ColorEditOptionsPopup(ImGuiColorEditFlags flags) {
    ...
    if (allow_opt_inputs) {
        if (ImGui::RadioButton("RGB", (opts & ImGuiColorEditFlags_RGB) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__InputsMask) |
                   ImGuiColorEditFlags_RGB;
        if (ImGui::RadioButton("HSV", (opts & ImGuiColorEditFlags_HSV) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__InputsMask) |
                   ImGuiColorEditFlags_HSV;
        if (ImGui::RadioButton("HEX", (opts & ImGuiColorEditFlags_HEX) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__InputsMask) |
    ...
    if (allow_opt_datatype) {
        if (allow_opt_inputs)
            ImGui::Separator();
        if (ImGui::RadioButton("0..255",
                               (opts & ImGuiColorEditFlags_Uint8) ? 1 : 0))
            opts = (opts & ~ImGuiColorEditFlags__DataTypeMask) |
                   ImGuiColorEditFlags_Uint8;
        if (ImGui::RadioButton("0.00..1.00",
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, float *ref_col) {
    ...
    if (allow_opt_picker) {
        ImVec2 picker_size(
            g.FontSize * 8,
            ImMax(g.FontSize * 8 -
                      (ColorSquareSize() + g.Style.ItemInnerSpacing.x),
...
bool ImGui::ColorEdit4(const char *label, float col[4],
    ...
    const float w_extra = (flags & ImGuiColorEditFlags_NoSmallPreview)
                              ? 0.0f
                              : (ColorSquareSize() + style.ItemInnerSpacing.x);
...
bool ImGui::ColorPicker4(const char *label, float col[4],
    ...
    float bars_width =
        ColorSquareSize(); // Arbitrary smallish width of Hue/Alpha picking bars
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview)) {
        ...
        if (ref_col != NULL) {
            ...
            if (ColorButton("##original", ref_col_v4,
                            ...
                            ImVec2(square_sz * 3, square_sz * 2))) {
                memcpy(col, ref_col,
                       ((flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4) *
                           sizeof(float));
                value_changed = true;
            }
        }
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        for (int n = 0; n < 6; n++) {
            ...
            PaintVertsLinearGradientKeepAlpha(
                ...
                draw_list->_VtxWritePtr, gradient_p0, gradient_p1,
...
void ImGui::EndGroup() {
    ...
    // copy it to LastItemId so IsItemActive() will function on the entire
...
float ImGui::GetColumnOffset(int column_index) {
    ...
    {
        ImGuiContext& g = *GImGui;
        const ImGuiID column_id = window->DC.ColumnsSetId +
    ImGuiID(column_index); if (g.ActiveId == column_id) return
    GetDraggedColumnOffset(column_index);
    }
    */
...
void ImGui::SetColumnOffset(int column_index, float offset) {
    ...
    if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoForceWithinWindow))
        offset =
            ImMin(offset, window->DC.ColumnsMaxX -
                              g.Style.ColumnsMinSpacing *
...
void ImGui::BeginColumns(const char *id, int columns_count,
    ...
    for (int column_index = 0; column_index < columns_count; column_index++) {
        float clip_x1 = ImFloor(0.5f + window->Pos.x +
                                GetColumnOffset(column_index) - 1.0f);
        ...
        window->DC.ColumnsData[column_index].ClipRect.ClipWith(
            window->ClipRect);
    }
            ...
            if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoResize)) {
                ...
                if (held && g.ActiveIdIsJustActivated)
                    g.ActiveIdClickOffset.x -=
                        column_w; // Store from center of column line (we used a
                                  // 8 wide rect for columns clicking). This is
                                  // used by GetDraggedColumnOffset().
                if (held)
                    dragging_column = i;
            }
    ...
    if (ImGui::Begin("ImGui Metrics", p_open)) {
        ...
        struct Funcs {
                        ...
                        for (int prim = clipper.DisplayStart,
                            ...
                            if (ImGui::IsItemHovered())
                                overlay_draw_list->AddPolyline(
                                    ...
                                    false); // Add triangle without AA, more
                                            // readable for large-thin triangle
                        }
                    ImGui::TreePop();
                }
            ...
            static void NodeWindow(ImGuiWindow *window, const char *label) {
                ...
                ImGui::BulletText("Pos: (%.1f,%.1f)", window->Pos.x,
                                  window->Pos.y);
        ...
        if (ImGui::TreeNode("Basic state")) {
            ...
            ImGui::Text("HoveredId: 0x%08X/0x%08X", g.HoveredId,
                        g.HoveredIdPreviousFrame); // Data is "in-flight" so
                                                   ...
                                                   // Metrics window is called
                                                   // we may see current frame
                                                   // information or not
