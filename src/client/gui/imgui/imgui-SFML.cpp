...
static sf::Texture *s_fontTexture = NULL; // owning pointer to internal font
                                          // atlas which is used if user doesn't
                                          // set custom sf::Texture.
namespace {
    ...
    bool imageButtonImpl(const sf::Texture &texture,
                         ...
                         const sf::Vector2f &size, const int framePadding,
...
namespace ImGui {
    namespace SFML {
        ...
        void Update(sf::Window &window, sf::RenderTarget &target, sf::Time dt) {
            ...
            if (!s_mouseMoved) {
                ...
                Update(s_touchPos, static_cast<sf::Vector2f>(target.getSize()),
                       dt);
            } else {
            ...
            window.setMouseCursorVisible(
                !ImGui::GetIO().MouseDrawCursor); // don't draw mouse cursor if
                                                  // ImGui draws it
        }
        ...
        void Update(const sf::Vector2i &mousePos,
            ...
            if (s_windowHasFocus) {
                ...
                for (unsigned int i = 0; i < 3; i++) {
                    io.MouseDown[i] =
                        s_touchDown[i] || sf::Touch::isDown(i) ||
                        s_mousePressed[i] ||
            ...
            assert(io.Fonts->Fonts.Size > 0); // You forgot to create and set up
                                              // font atlas (see
                                              // createFontTexture)
            ImGui::NewFrame();
        }
        ...
        void Shutdown() {
            ...
            if (s_fontTexture) { // if internal texture was created, we delete
                                 // it
                delete s_fontTexture;
                s_fontTexture = NULL;
            }
    ...
    void Image(const sf::Texture &texture, const sf::FloatRect &textureRect,
               const sf::Color &tintColor, const sf::Color &borderColor) {
        Image(texture,
              sf::Vector2f(std::abs(textureRect.width),
    ...
    void Image(const sf::Texture &texture, const sf::Vector2f &size,
        ...
        ImGui::Image((void *)texture.getNativeHandle(), size, uv0, uv1,
    ...
    bool ImageButton(const sf::Texture &texture, const int framePadding,
                     const sf::Color &bgColor, const sf::Color &tintColor) {
        return ImageButton(texture,
                           static_cast<sf::Vector2f>(texture.getSize()),
    ...
    bool ImageButton(const sf::Texture &texture, const sf::Vector2f &size,
        ...
        return ::imageButtonImpl(
            texture, sf::FloatRect(0.f, 0.f, textureSize.x, textureSize.y),
    ...
    bool ImageButton(const sf::Sprite &sprite, const sf::Vector2f &size,
        ...
        return ::imageButtonImpl(
            *texturePtr, static_cast<sf::FloatRect>(sprite.getTextureRect()),
            size, framePadding, bgColor, tintColor);
    }
...
namespace {
    ...
    void RenderDrawLists(ImDrawData *draw_data) {
        ...
        assert(io.Fonts->TexID !=
               NULL); // You forgot to create and set font texture
        ...
        for (int n = 0; n < draw_data->CmdListsCount; ++n) {
            ...
            for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.size(); ++cmd_i) {
                ...
                if (pcmd->UserCallback) {
                    ...
                    GLuint tex_id =
                        (GLuint) * ((unsigned int *)&pcmd->TextureId);
    ...
    bool imageButtonImpl(const sf::Texture &texture,
        ...
        return ImGui::ImageButton((void *)texture.getNativeHandle(), size, uv0,
                                  uv1, framePadding, bgColor, tintColor);
    }
